...
export const incrementarVistasService = async (id: string) => {
    try {
        const snapshot = await db.collection("HistoriaInfo").where("id", "==", id).get();

        if (snapshot.empty) {
            console.warn(`⚠️ Historia con id ${id} no encontrada para incrementar vistas.`);
            return null;
        }
        const docRef = snapshot.docs[0].ref;
        const vistasActuales = snapshot.docs[0].data().vistas || 0;

        await docRef.update({
            vistas: fieldValue.increment(1)
        });

        console.log(`✅ Vistas incrementadas para ${id}. Total: ${vistasActuales + 1}`);
        return vistasActuales + 1;
    } catch (error) {
        console.error(`❌ Error en incrementarVistasService (ID: ${id}):`, error);
        throw error;
    }
};
export const actualizarLikesHelper = async (idPublicacion: string, operacion: "sumar" | "restar") => {
    try {
        const snapshot = await db.collection("HistoriaInfo").where("id", "==", idPublicacion).get();
        if (!snapshot.empty) {
