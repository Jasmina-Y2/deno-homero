...
export const seguirUsuarioService = async (miUid: string, uidASeguir: string) => {
    try {
...
export const dejarDeSeguirService = async (miUid: string, uidADejar: string) => {
    try {
        const seguirUserRef = db.collection("Seguir_User").doc(miUid);
        const seguirRef = db.collection("Seguir").doc(uidADejar);

        await seguirUserRef.update({
            siguiendo: fieldValue.arrayRemove(uidADejar),
        });

        await seguirRef.update({
