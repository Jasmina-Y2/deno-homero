...
export const guardarComentario = async (ctx: RouterContext<string>) => {
    try {
        ...
        ctx.response.body = { success: false, error: error instanceof Error ? error.message : "Error al guardar comentario" };
    }
};
...
export const obtenerComentarios = async (ctx: RouterContext<string>) => {
    try {
        ...
        ctx.response.status = 500;
        ctx.response.body = { success: false, error: error instanceof Error ? error.message : "Error al obtener comentarios" };
    }
};