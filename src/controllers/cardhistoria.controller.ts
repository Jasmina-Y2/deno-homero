...
import { guardarCardHistoriaEnFirestoreService, obtenerCardHistoriaService, getHistoriaCardByCustomId2Service, eliminarCardPorIdService, eliminarImagenesDeHistoria } from "../service/cardhistoria.service.ts";
...
export const eliminarCardController = async (ctx: RouterContext<string>) => {
    try {
        ...
        if (eliminado) {
            ctx.response.status = 200;
            ctx.response.body = { success: true, message: "Historia eliminada correctamente" };
        } else {
            ctx.response.status = 404;
            ctx.response.body = { success: false, message: "No se encontró ninguna historia con ese ID" };
        }
...
export const eliminarMultimediaController = async (ctx: RouterContext<string>) => {
    try {
