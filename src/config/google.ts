...
export async function getGoogleCredentials() {
  ...
  const envCredentials = Deno.env.get("GOOGLE_APPLICATION_CREDENTIALS_JSON") ||
...
const { credentials: initialCredentials, path: foundPath } = await getGoogleCredentials();

if (initialCredentials) {
  console.log(`🤖 Google Cloud TTS configurado con credenciales de: ${foundPath} (${initialCredentials.client_email})`);
} else {
...
const getS3Mp3Url = (name: string) =>
  `https://${BUCKET_NAME}.s3.us-east-1.amazonaws.com/${FOLDER_GOOGLE_VOCES}/${encodeURIComponent(name)}.mp3`;

/**
 * Las 30 voces oficiales de Google (Gemini / Chirp3-HD) con preview_url y mp3
 */
export const GOOGLE_OFFICIAL_VOICES = [
  { shortName: "Aoede", gender: "female", desc: "Voz femenina expresiva, brillante y juvenil" },
  { shortName: "Puck", gender: "male", desc: "Voz masculina conversacional, alegre y natural" },
  { shortName: "Charon", gender: "male", desc: "Voz masculina profunda, reflexiva y calmada" },
  { shortName: "Fenrir", gender: "male", desc: "Voz masculina potente, firme y elegante" },
  { shortName: "Kore", gender: "female", desc: "Voz femenina dulce, cálida y tranquilizadora" },
  { shortName: "Leda", gender: "female", desc: "Voz femenina suave, relajante y clara" },
  { shortName: "Orus", gender: "male", desc: "Voz masculina directa, dinámica y profesional" },
  { shortName: "Zephyr", gender: "female", desc: "Voz femenina moderna, fluida y amigable" },
  { shortName: "Achernar", gender: "female", desc: "Voz femenina refinada de alta definición" },
  { shortName: "Achird", gender: "male", desc: "Voz masculina clásica y articulada" },
  { shortName: "Algenib", gender: "male", desc: "Voz masculina enérgica y confiable" },
  { shortName: "Algieba", gender: "male", desc: "Voz masculina con tono cálido" },
  { shortName: "Alnilam", gender: "male", desc: "Voz masculina narrativa envolvente" },
  { shortName: "Autonoe", gender: "female", desc: "Voz femenina melódica" },
  { shortName: "Callirrhoe", gender: "female", desc: "Voz femenina serena y equilibrada" },
  { shortName: "Despina", gender: "female", desc: "Voz femenina expresiva para historias" },
  { shortName: "Enceladus", gender: "male", desc: "Voz masculina resonante para locución" },
  ...
  { shortName: "Pulcherrima", gender: "female", desc: "Voz femenina inspiradora" },
  ...
  { shortName: "Umbriel", gender: "male", desc: "Voz masculina misteriosa y profunda" },
  { shortName: "Vindemiatrix", gender: "female", desc: "Voz femenina vivaz" },
  { shortName: "Zubenelgenubi", gender: "male", desc: "Voz masculina distintiva" },
] as const;

export const GOOGLE_VOICES_CATALOG: GoogleVoiceMetadata[] = GOOGLE_OFFICIAL_VOICES.map((v) => {
 ...
 */
export const GOOGLE_VOICES_SHORT_MAP: Record<string, { fullName: string; languageCode: string; gender: "female" | "male"; shortName: string }> = {};
 ...
 */
export function extractLanguageCode(voiceName: string, fallback = "es-US"): string {
